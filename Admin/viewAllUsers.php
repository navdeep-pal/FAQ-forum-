...
$sql = "SELECT * FROM user where active=1 ORDER BY created_at DESC";
$stmt = $pdo->prepare($sql);
$stmt->execute();
$users = $stmt->fetchAll(PDO::FETCH_ASSOC);
$sql = "SELECT * FROM user where active=2 ORDER BY created_at DESC";
$stmt = $pdo->prepare($sql);
$stmt->execute();
...
<body>
    ...
    <header class="admin-header">
        <div class="container d-flex justify-content-between align-items-center">
            ...
            <div>
                ...
                <a href="viewDeactivatedUser.php" class="btn btn-outline-light btn-sm me-2 rounded-pill">
                    <i class="fas fa-user-plus me-1"></i> Deactivated Users
                </a>
    ...
    <main class="container">
        ...
        <div class="table-container mt-4">
            <div class="table-responsive">
                <h4>New Users (Activation Requests)</h4>
                <table class="table table-striped table-hover">
                    <thead class="table-dark">
                        <tr>
                            <th>Photo</th>
                            ...
                            <?php foreach ($newusers as $user): ?>
                                <tr>
                                    ...
                                    <td class="action-btns text-center">
                                        ...
                                        <button type="button" class="btn btn-sm btn-delete" 
                                                data-bs-toggle="modal" data-bs-target="#activateConfirmModal"
                                                data-user-name="<?= htmlspecialchars($user['name']) ?>"
                                                data-user-cpf="<?= htmlspecialchars($user['cpf_no']) ?>">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded-3">
                ...
                <div class="custom-modal-body">
                    Are you sure you want to deactivate user "<strong id="modalUserName"></strong>" (CPF: <strong id="modalUserCpf"></strong>)? This action cannot be undone.
                </div>
                <div class="custom-modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <form action="executeDeleteUser.php" method="POST" class="d-inline" id="deleteUserForm">
                        <input type="hidden" name="user_cpf" id="deleteUserCpfInput">
                        <button type="submit" class="btn btn-danger btn-delete">Deactivate</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="activateConfirmModal" tabindex="-1" aria-labelledby="deleteConfirmModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded-3">
                <div class="custom-modal-header">
    ...
    <script>
        ...
        var deleteConfirmModal = document.getElementById('activateConfirmModal');
        deleteConfirmModal.addEventListener('show.bs.modal', function (event) {
            // Button that triggered the modal
            var button = event.relatedTarget; 
            // Extract info from data-bs-* attributes
